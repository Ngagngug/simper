...
class VerifikasitokoobatsController extends \BaseController {
	...
	{
		$matchThese = ['perijinan_id' => '10','verifikasi' => 'Proses Verifikasi'];
		$orThose = ['perijinan_id' => '10','verifikasi' => 'Verifikasi Belum Lengkap'];
		if(Datatable::shouldHandle())
	    {
	           return Datatable::collection(Pengguna::where($matchThese)->orwhere($orThose)->get())
	            ...
	            ->addColumn('', function ($model) {
                    $html = '<center> <a href="'.route('admin.verifikasitokoobats.edit', ['verifikasitokoobats'=>$model->id]).'" class="btn btn-sm btn-default">Verifikasi</a> ';
					$html .= Form::open(array('url' => route('admin.verifikasitokoobats.destroy', ['verifikasitokoobats'=>$model->id]), 'method'=>'delete', 'class'=>'inline js-confirm'));
		...
		return View::make('verifikasitokoobats.index')->withTitle('Verifikasi Toko Obat');
	}
	...
	{
		return View::make('verifikasitokoobats.create')->withTitle('Tambah Verifikasi');
	}
	...
	public function store()
	{
		...
		return Redirect::route('admin.verifikasitokoobats.index')->with("successMessage", "Berhasil menyimpan $verifikasi->verifikasi ");
	...
	public function show($id)
	{
		...
		return View::make('verifikasitokoobats.show', compact('verifikasi'));
	...
	{	
	    ...
	    $data = Verifikasitokoobat::all();

		// return View::make('devices', compact(['locations', 'devices']);

		return View::make('verifikasitokoobats.edit', ['verifikasi'=>$verifikasi, 'data'=>$data])->withTitle("Verifikasi $verifikasi->nama");
	...
	public function destroy($id)
	{
		...
		return Redirect::route('admin.verifikasitokoobats.index')->with('successMessage', 'User berhasil dihapus.');
    ...
    {
        return View::make('verifikasitokoobats.export')->withTitle('Export Data');
    }
    ...
    public function exportPost()
    {
        ...
        $verifikasi = Pengguna::where('perijinan_id', '10')->whereIn('verifikasi',Input::get('verifikasi'))->get();
    ...
    private function exportPdf($verifikasi)
    {
        ...
        return $pdf->download('verifikasitokoobats.pdf');
    }
