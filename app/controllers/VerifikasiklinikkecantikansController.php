...
class VerifikasiklinikkecantikansController extends \BaseController {
	...
	{
		$matchThese = ['perijinan_id' => '6','verifikasi' => 'Proses Verifikasi'];
		$orThose = ['perijinan_id' => '6','verifikasi' => 'Verifikasi Belum Lengkap'];
	    ...
	    //      return Datatable::collection(Pengguna::where('perijinan_id','4')->get())
	           return Datatable::collection(Pengguna::where($matchThese)->orwhere($orThose)->get())
	            ...
	            ->addColumn('', function ($model) {
                    $html = '<center> <a href="'.route('admin.verifikasiklinikkecantikans.edit', ['verifikasiklinikkecantikans'=>$model->id]).'" class="btn btn-sm btn-default">Verifikasi</a> ';
					$html .= Form::open(array('url' => route('admin.verifikasiklinikkecantikans.destroy', ['verifikasiklinikkecantikans'=>$model->id]), 'method'=>'delete', 'class'=>'inline js-confirm'));
		...
		return View::make('verifikasiklinikkecantikans.index')->withTitle('Verifikasi Klinik Kecantikan');
	}
	...
	{
		return View::make('verifikasiklinikkecantikans.create')->withTitle('Tambah Verifikasi');
	}
	...
	public function store()
	{
		...
		return Redirect::route('admin.verifikasiklinikkecantikans.index')->with("successMessage", "Berhasil menyimpan $verifikasi->verifikasi ");
	...
	public function show($id)
	{
		...
		return View::make('verifikasiklinikkecantikans.show', compact('verifikasi'));
	}
	...
	public function edit($id)
	{
	    ...
	    $data = Verifikasiklinikkecantikan::all();

		// return View::make('devices', compact(['locations', 'devices']);

		return View::make('verifikasiklinikkecantikans.edit', ['verifikasi'=>$verifikasi, 'data'=>$data])->withTitle("Verifikasi $verifikasi->nama");
	...
	public function update($id)
	{
		...
		if( $count==count($data)) {
			...
			$verifikasi->verifikasi = "Proses Visitasi";
		 	...
		 	Mail::send('verifikasiklinikkecantikans.email.messagesuccess',$syaratString, function($message) use($verifikasi){
			...
			Mail::send('verifikasiklinikkecantikans.email.messagefailed', $syaratString, function($message) use($verifikasi){
	...
	public function destroy($id)
	{
		...
		return Redirect::route('admin.verifikasiklinikkecantikans.index')->with('successMessage', 'User berhasil dihapus.');
	}
    ...
    {
        return View::make('verifikasiklinikkecantikans.export')->withTitle('Export Data');
    }
    ...
    public function exportPost()
    {
        ...
        $verifikasi = Pengguna::where('perijinan_id', '6')->whereIn('verifikasi',Input::get('verifikasi'))->get();
    ...
    private function exportPdf($verifikasi)
    {
        ...
        $pdf = PDF::loadView('pdf.verifikasiklinikkecantikans', $data);
        return $pdf->download('verifikasiklinikkecantikans.pdf');
    }
