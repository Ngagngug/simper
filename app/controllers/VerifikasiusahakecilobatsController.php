...
class VerifikasiusahakecilobatsController extends \BaseController {
	...
	{
		$matchThese = ['perijinan_id' => '14','verifikasi' => 'Proses Verifikasi'];
		$orThose = ['perijinan_id' => '14','verifikasi' => 'Verifikasi Belum Lengkap'];
		if(Datatable::shouldHandle())
	    {
	           return Datatable::collection(Pengguna::where($matchThese)->orwhere($orThose)->get())
	            ...
	            ->addColumn('', function ($model) {
                    $html = '<center> <a href="'.route('admin.verifikasiusahakecilobats.edit', ['verifikasiusahakecilobats'=>$model->id]).'" class="btn btn-sm btn-default">Verifikasi</a> ';
					$html .= Form::open(array('url' => route('admin.verifikasiusahakecilobats.destroy', ['verifikasiusahakecilobats'=>$model->id]), 'method'=>'delete', 'class'=>'inline js-confirm'));
		...
		return View::make('verifikasiusahakecilobats.index')->withTitle('Verifikasi Usaha Kecil Obat');
	}
	...
	{
		return View::make('verifikasiusahakecilobats.create')->withTitle('Tambah Verifikasi');
	}
	...
	public function store()
	{
		...
		return Redirect::route('admin.verifikasiusahakecilobats.index')->with("successMessage", "Berhasil menyimpan $verifikasi->verifikasi ");
	...
	public function show($id)
	{
		...
		return View::make('verifikasiusahakecilobats.show', compact('verifikasi'));
	...
	{	
	    ...
	    $data = Verifikasiusahakecilobat::all();

		// return View::make('devices', compact(['locations', 'devices']);

		return View::make('verifikasiusahakecilobats.edit', ['verifikasi'=>$verifikasi, 'data'=>$data])->withTitle("Verifikasi $verifikasi->nama");
	...
	public function destroy($id)
	{
		...
		return Redirect::route('admin.verifikasiusahakecilobats.index')->with('successMessage', 'User berhasil dihapus.');
    ...
    {
        return View::make('verifikasiusahakecilobats.export')->withTitle('Export Data');
    }
    ...
    public function exportPost()
    {
        ...
        $verifikasi = Pengguna::where('perijinan_id', '14')->whereIn('verifikasi',Input::get('verifikasi'))->get();
    ...
    private function exportPdf($verifikasi)
    {
        ...
        return $pdf->download('verifikasiusahakecilobats.pdf');
    }
