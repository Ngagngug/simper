...
class VerifikasiunittransfusidarahsController extends \BaseController {
	...
	{
		$matchThese = ['perijinan_id' => '16','verifikasi' => 'Proses Verifikasi'];
		$orThose = ['perijinan_id' => '16','verifikasi' => 'Verifikasi Belum Lengkap'];
		if(Datatable::shouldHandle())
	    {
	           return Datatable::collection(Pengguna::where($matchThese)->orwhere($orThose)->get())
	            ...
	            ->addColumn('', function ($model) {
                    $html = '<center> <a href="'.route('admin.verifikasiunittransfusidarahs.edit', ['verifikasiunittransfusidarahs'=>$model->id]).'" class="btn btn-sm btn-default">Verifikasi</a> ';
					$html .= Form::open(array('url' => route('admin.verifikasiunittransfusidarahs.destroy', ['verifikasiunittransfusidarahs'=>$model->id]), 'method'=>'delete', 'class'=>'inline js-confirm'));
		...
		return View::make('verifikasiunittransfusidarahs.index')->withTitle('Verifikasi Unit Transfusi Darah');
	}
	...
	{
		return View::make('verifikasiunittransfusidarahs.create')->withTitle('Tambah Verifikasi');
	}
	...
	public function store()
	{
		...
		return Redirect::route('admin.verifikasiunittransfusidarahs.index')->with("successMessage", "Berhasil menyimpan $verifikasi->verifikasi ");
	...
	public function show($id)
	{
		...
		return View::make('verifikasiunittransfusidarahs.show', compact('verifikasi'));
	...
	{	
	    ...
	    $data = Verifikasiunittransfusidarah::all();

		// return View::make('devices', compact(['locations', 'devices']);

		return View::make('verifikasiunittransfusidarahs.edit', ['verifikasi'=>$verifikasi, 'data'=>$data])->withTitle("Verifikasi $verifikasi->nama");
	...
	public function destroy($id)
	{
		...
		return Redirect::route('admin.verifikasiunittransfusidarahs.index')->with('successMessage', 'User berhasil dihapus.');
    ...
    {
        return View::make('verifikasiunittransfusidarahs.export')->withTitle('Export Data');
    }
    ...
    public function exportPost()
    {
        ...
        $verifikasi = Pengguna::where('perijinan_id', '16')->whereIn('verifikasi',Input::get('verifikasi'))->get();
    ...
    private function exportPdf($verifikasi)
    {
        ...
        return $pdf->download('verifikasiunittransfusidarahs.pdf');
    }
