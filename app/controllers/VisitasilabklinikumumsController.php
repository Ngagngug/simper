...
class VisitasilabklinikumumsController extends \BaseController {
	...
	{
		$matchThese = ['perijinan_id' => '7','verifikasi' => 'Visitasi Selesai'];
		$orThose = ['perijinan_id' => '7','verifikasi' => 'Visitasi Gagal'];
		$orThoses = ['perijinan_id' => '7','verifikasi' => 'Proses Visitasi'];
		if(Datatable::shouldHandle())
	    {
	     	 return Datatable::collection(Pengguna::where($matchThese)->orwhere($orThose)->orwhere($orThoses)->get())
	            ...
	            ->addColumn('', function ($model) {
                    $html = '<center> <a href="'.route('admin.visitasilabklinikumums.edit', ['visitasilabklinikumums'=>$model->id]).'" class="btn btn-sm btn-default">Visitasi</a> ';
	...
	{
		return View::make('visitasilabklinikumums.create')->withTitle('Tambah Visitasi');
	}
	...
	public function store()
	{
		...
		return Redirect::route('admin.visitasivisitasilabklinikumumskliniks.index')->with("successMessage", "Berhasil menyimpan $verifikasi->verifikasi ");
	}
	...
	public function edit($id)
	{
		...
		return View::make('visitasilabklinikumums.edit', ['verifikasi'=>$verifikasi, 'data'=>$data])->withTitle("Verifikasi $verifikasi->nama");
	...
	public function update($id)
	{
		...
		if( $count==count($data)) {
	        ...
	        return $pdf->download($verifikasi->nama.'_visitasilabklinikumums.pdf');
		...
		return Redirect::route('admin.visitasilabklinikumums.index')->with("successMessage", "Berhasil menyimpan $verifikasi->verifikasi" );
	}
	...
	public function destroy($id)
	{
		...
		return Redirect::route('admin.visitasilabklinikumums.index')->with('successMessage', 'User berhasil dihapus.');
    ...
    {
        return View::make('visitasilabklinikumums.export')->withTitle('Export Data');
    }
    ...
    private function exportPdf($verifikasi)
    {
        ...
        return $pdf->download('visitasilabklinikumums.pdf');
    }
