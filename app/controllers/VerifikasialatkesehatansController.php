...
class VerifikasialatkesehatansController extends \BaseController {
	...
	{
		$matchThese = ['perijinan_id' => '11','verifikasi' => 'Proses Verifikasi'];
		$orThose = ['perijinan_id' => '11','verifikasi' => 'Verifikasi Belum Lengkap'];
		if(Datatable::shouldHandle())
	    {
	           return Datatable::collection(Pengguna::where($matchThese)->orwhere($orThose)->get())
	            ...
	            ->addColumn('', function ($model) {
                    $html = '<center> <a href="'.route('admin.verifikasialatkesehatans.edit', ['verifikasialatkesehatans'=>$model->id]).'" class="btn btn-sm btn-default">Verifikasi</a> ';
					$html .= Form::open(array('url' => route('admin.verifikasialatkesehatans.destroy', ['verifikasialatkesehatans'=>$model->id]), 'method'=>'delete', 'class'=>'inline js-confirm'));
		...
		return View::make('verifikasialatkesehatans.index')->withTitle('Verifikasi Toko Alat Kesehatan');
	}
	...
	{
		return View::make('verifikasialatkesehatans.create')->withTitle('Tambah Verifikasi');
	}
	...
	public function store()
	{
		...
		return Redirect::route('admin.verifikasialatkesehatans.index')->with("successMessage", "Berhasil menyimpan $verifikasi->verifikasi ");
	...
	public function show($id)
	{
		...
		return View::make('verifikasialatkesehatans.show', compact('verifikasi'));
	...
	public function edit($id)
	{
	    ...
	    $data = Verifikasialatkesehatan::all();

		// return View::make('devices', compact(['locations', 'devices']);

		return View::make('verifikasialatkesehatans.edit', ['verifikasi'=>$verifikasi, 'data'=>$data])->withTitle("Verifikasi $verifikasi->nama");
	...
	public function destroy($id)
	{
		...
		return Redirect::route('admin.verifikasialatkesehatans.index')->with('successMessage', 'User berhasil dihapus.');
    ...
    {
        return View::make('verifikasialatkesehatans.export')->withTitle('Export Data');
    }
    ...
    public function exportPost()
    {
        ...
        $verifikasi = Pengguna::where('perijinan_id', '11')->whereIn('verifikasi',Input::get('verifikasi'))->get();
    ...
    private function exportPdf($verifikasi)
    {
        ...
        return $pdf->download('verifikasialatkesehatans.pdf');
    }
